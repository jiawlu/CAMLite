...
def _findPathForAgents(demands, net, netgraph, iteration_no):
    for agent in demands.agent_list:
        ...
        for node_id in path: agent.meso_path_node_seq_no_list.append(net.meso_node_id_to_seq_no_dict[node_id])
        number_of_nodes_in_path = len(path)
        for i in range(1, number_of_nodes_in_path): agent.meso_path_link_seq_no_list.append(net.meso_link_key_to_seq_no_dict[path[i - 1] * 100000 + path[i]])
